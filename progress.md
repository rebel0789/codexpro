...
| Goal card resource/cache contract | v13 stale cache → v14 URI fix → v15 changed-count UI → v16 turn-history UI | Every renderer payload change gets a fresh URI | Real Phase 8 reconnect mounted v14; real Phase 9 final get/review mounted v16 with 2/2 turn history; v15–v8 remain legacy | PASS |
...
| Question | Answer |
|----------|--------|
| Where am I? | Supervised Isolated/Live, Phase 8 one-turn Persistent, and Phase 9 bounded continuation flows are verified through ordinary Chat on POSIX; built HTTP/MCP and installed-real-Codex paths also pass |
| Where am I going? | Fresh bounded retries remain roadmap work, and Goal orchestration remains unavailable on Windows |
| What's the goal? | Deliver a real Pro-orchestrated local Goal vertical slice |
| What have I learned? | See `findings.md` |
| What have I done? | Product contract, real CodingTask flow, supervised Isolated/Live Goals, Persistent scheduling/recovery/interruption, bounded same-thread continuations, final-only integration, installed-real-Codex HTTP/MCP, and the versioned v16 card path are complete |
