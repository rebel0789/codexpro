...
- Verified the complete supervised Live happy path in ordinary Chat through the installed private plugin and a real `gpt-5.6-sol`/`high` Codex App Server turn: exact integration review, separately approved projection, completion, zero-write final adoption, unchanged source HEAD, and no stage, commit, push, or PR. The v13 default sandbox-fallback card rendered without increasing template errors; explicit endpoint-domain card fetches remain intermittently unreliable.
...
- Added Goal HTTP/MCP and execution regressions covering parallel detached workers, reconnect recovery, passive reads, isolated integration, cancellation, idempotent source application, and preservation of unrelated source dirt. Unsupported persistent, multi-turn, and automatic-retry contracts fail closed.
