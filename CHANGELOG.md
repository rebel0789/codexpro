...
- Added Goal HTTP/MCP and execution regressions covering parallel detached workers, reconnect recovery, passive reads, isolated integration, cancellation, idempotent source application, preservation of unrelated source dirt, same-thread bounded continuations, intermediate integration/dependency gates, and final-only cumulative integration. Fresh automatic-retry contracts and invalid continuation counts still fail closed.
- Upgraded the opt-in Apps widget to v16 with Goal lifecycle, scheduler state, authoritative changed-file counts, approval, work graph, Blackboard, bounded continuation/turn history, review, and next-action rendering. A stale v13 cache established the rule that renderer changes require a new resource URI; v14 carried the initial fix, the count UI moved to v15, and the new turn-history payload moved the current resource to v16 while v15 through v8 remain compatible legacy resources.
