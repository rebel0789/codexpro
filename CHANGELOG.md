...
- Added Goal HTTP/MCP and execution regressions covering parallel detached workers, reconnect recovery, passive reads, isolated integration, cancellation, idempotent source application, and preservation of unrelated source dirt. Persistent is now covered; unsupported multi-turn and fresh automatic-retry contracts still fail closed.
- Upgraded the opt-in Apps widget to v15 with Goal lifecycle, scheduler state, authoritative changed-file counts, approval, work graph, Blackboard, review, and next-action rendering. A stale v13 cache established the rule that renderer changes require a new resource URI; v14 carried the initial fix, and the subsequent count UI moved the current resource to v15 while v14 through v8 remain compatible legacy resources.
