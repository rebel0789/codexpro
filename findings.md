...
- The real v13 default sandbox-fallback card rendered without increasing the template error count, but Chat could retain stale renderer bytes when a payload changed under the same resource URI. The first cache-safe fix moved to v14, the authoritative changed-file-count UI moved to v15, and the bounded turn-history payload moved the current resource to v16. v15 through v8 remain compatibility resources; the final Phase 9 ordinary-Chat get/review cards mounted v16 successfully.
...
- Phase 9 adds only approval-time bounded continuations to Persistent: 1–4 total turns including the initial turn, with exactly one fewer ordered immutable intent and no skip. Supervised stays single-turn, and fresh automatic retries remain zero.
...
| Keep continuation distinct from retry | A continuation is approved semantic work on the same task/thread/session; failure stops, while same-operation recovery restores only the original operation and fresh attempts remain unimplemented |
...
- In the real Phase 8 Persistent flow, reconnect mounted the v14 Goal card after the user navigated away, and the authoritative tools showed the scheduler stopped for semantic review. Changed-file-count rendering moved to v15 and bounded Phase 9 turn history moved to current v16.
