...
try {
  ...
  await fs.writeFile(path.join(sourceRoot, 'src', 'dependent.txt'), 'dependent base\n');
...
`;
  ...
  let firstRunBefore;
  if (!realMode) {
    const turnTwoReady = await poll(async () => ({ events: await protocolEvents(), state: await fs.readFile(goalStatePath, 'utf8').then(JSON.parse) }), (value) => value.state.lifecycle === 'failed' || value.events.some((event) => event.kind === 'turn_start' && event.operationId.endsWith(':turn:2:attempt:0')), 30_000);
    ...
    assert.deepEqual([turnOne.operationId, turnTwo.operationId], [`goal:${goalId.slice(5)}:work_multiturn:turn:1:attempt:0`, `goal:${goalId.slice(5)}:work_multiturn:turn:2:attempt:0`]);
    ...
    const events = await protocolEvents(); const resume = events.find((event) => event.kind === 'thread_resume'); const second = events.find((event) => event.kind === 'turn_start' && event.operationId.endsWith(':turn:2:attempt:0'));
  ...
  assert.deepEqual(work.turns.map((turn) => turn.operationId), [`goal:${goalId.slice(5)}:work_multiturn:turn:1:attempt:0`, `goal:${goalId.slice(5)}:work_multiturn:turn:2:attempt:0`]);
  assert.deepEqual(work.turns.map((turn) => turn.attempts.length), [1, 1]); assert.deepEqual(work.turns.map((turn) => turn.attempts[0].operationId), work.turns.map((turn) => turn.operationId));
  ...
  if (!realMode) {
    ...
    assert.equal((await fs.readdir(path.join(dataRoot, 'tasks', turnOne.taskId, 'continuations'))).length, 1, 'the sole approved continuation must have one immutable lineage decision');
  }
  ...
  if (!realMode) {
    // A continuation may not reuse a prior semantic turn identity, even if its content is otherwise valid.
    await fs.writeFile(path.join(gates, 'duplicate-turn-id'), '1'); const duplicateKey = 'persistent-multiturn-duplicate-id-v1';
    const duplicateProposal = await call('propose_goal', {
      workspace_id: opened.structuredContent.workspace_id, goal_key: duplicateKey, title: 'Reject duplicate semantic turn identity', goal: 'Reject a continuation that reuses its predecessor turn identity before any integration or dependency unlock.',
      completion_criteria: ['Duplicate turn identity is rejected without integration'], execution_policy: 'persistent', workspace_policy: 'isolated',
      limits: { max_concurrency: 1, timeout_ms: 30_000, max_turns_per_worker: 2, max_retries_per_worker: 2 },
      permissions: { file_globs: ['src/multi-turn.txt', 'src/dependent.txt'], commands: [], network: false, source_effects: { apply: false, commit: false, push: false, draft_pr: false } }, base_sha: baseSha,
      work: [
        { work_id: 'work_duplicate', title: 'Duplicate identity producer', goal: 'Replace only src/multi-turn.txt with exactly "turn one complete\\n".', acceptance_criteria: ['Turn one content exists'], file_globs: ['src/multi-turn.txt'], continuation_intents: [{ intent_id: 'duplicate_final', prompt: 'Read src/multi-turn.txt, preserve the first result, and replace only that file with exactly "turn one complete\\nturn two complete\\n".' }] },
        { work_id: 'work_dependent', title: 'Must remain blocked', goal: 'Replace only src/dependent.txt with exactly "dependent turn one\\n".', acceptance_criteria: ['Dependent content exists'], depends_on: ['work_duplicate'], file_globs: ['src/dependent.txt'], continuation_intents: [{ intent_id: 'dependent_final', prompt: 'Replace only src/dependent.txt with exactly "dependent turn one\\ndependent turn two\\n".' }] }
      ]
    });
    const duplicateApproved = await call('approve_goal', { goal_id: duplicateProposal.structuredContent.goal_id, expected_revision: duplicateProposal.structuredContent.revision, contract_fingerprint: duplicateProposal.structuredContent.contract_fingerprint, approval_key: `${duplicateKey}-approve`, confirm: true });
    const duplicateId = duplicateApproved.structuredContent.goal_id; const duplicatePath = path.join(dataRoot, 'goals', duplicateId, 'state.json');
    await call('start_goal', { goal_id: duplicateId, expected_revision: duplicateApproved.structuredContent.revision, start_key: `${duplicateKey}-start` });
    const rejected = await pollJson(duplicatePath, (state) => state?.lifecycle === 'failed' && state.scheduler?.status === 'stopped', 30_000); const duplicateWork = rejected.work.find((item) => item.workId === 'work_duplicate'); const dependentWork = rejected.work.find((item) => item.workId === 'work_dependent');
    assert.match(rejected.error, /previous semantic turn identity/); assert.equal(rejected.scheduler.stopReason, 'failed'); assert.equal(duplicateWork.turns.length, 2); assert.deepEqual(duplicateWork.turns.map((turn) => turn.attempts.length), [1, 1]); assert.equal(duplicateWork.turns[1].turnId, duplicateWork.turns[0].turnId);
    const identityFailure = duplicateWork.turns[1].attempts[0].failure; assert.equal(identityFailure.code, 'identity_mismatch'); assert.equal(identityFailure.category, 'identity'); assert.equal(identityFailure.phase, 'turn_start'); assert.equal(identityFailure.retryable, false); assert.equal(identityFailure.outcomeKnown, false); assert.equal(identityFailure.turnStarted, true);
    const duplicateEvents = (await protocolEvents()).filter((event) => event.kind === 'turn_start' && event.operationId.includes(duplicateId.slice(5))); assert.equal(duplicateEvents.length, 2); assert.equal(duplicateEvents[1].turnId, duplicateEvents[0].turnId, 'fixture must submit the adversarial duplicate turn identity');
    assert.equal(dependentWork.status, 'planned'); assert.equal(dependentWork.turns, undefined); assert.equal(gitAt(rejected.integrationWorktreeRoot, ['rev-parse', 'HEAD']), baseSha); assert.equal(gitAt(rejected.integrationWorktreeRoot, ['rev-list', '--count', `${baseSha}..HEAD`]), '0');
    await assert.rejects(fs.stat(path.join(dataRoot, 'goals', duplicateId, 'integrations', 'work_duplicate.json')), { code: 'ENOENT' }); await assert.rejects(fs.stat(path.join(dataRoot, 'goals', duplicateId, 'integrations', 'work_dependent.json')), { code: 'ENOENT' });
    const rejectedBytes = await fs.readFile(duplicatePath); const rejectedProtocol = await fs.readFile(protocolLog); await delay(1_200); assert.deepEqual(await fs.readFile(duplicatePath), rejectedBytes, 'identity failure must not reserve a retry'); assert.deepEqual(await fs.readFile(protocolLog), rejectedProtocol, 'identity failure must not relaunch a worker');
    const duplicateReview = await call('review_goal', { goal_id: duplicateId }); assert.equal(duplicateReview.structuredContent.review.changedFileCount, 0); assert.deepEqual(duplicateReview.structuredContent.review.changedPaths, []); assert.equal(duplicateReview.structuredContent.review.diff, '');
    assert.deepEqual(await sourceAuthority(), sourceBefore, 'duplicate turn identity rejection must preserve source authority');
  }
...
async function sourceAuthority() { return { head: git(['rev-parse', 'HEAD']), refs: git(['show-ref']), status: git(['status', '--porcelain=v2', '--untracked-files=all']), diff: git(['diff', '--binary']), staged: git(['diff','--cached','--binary']), tree: git(['write-tree']), source: await fs.readFile(path.join(sourceRoot, 'src', 'multi-turn.txt'), 'base64'), dependent: await fs.readFile(path.join(sourceRoot, 'src', 'dependent.txt'), 'base64'), dirty: await fs.readFile(path.join(sourceRoot, 'dirty.txt'), 'base64'), stagedFile: await fs.readFile(path.join(sourceRoot, 'staged.txt'), 'base64'), untracked: await fs.readFile(path.join(sourceRoot, 'untracked.txt'), 'base64') }; }
