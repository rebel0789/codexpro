...
## Cloudflare quick tunnel 的 URL 为什么在 ChatGPT 报错或返回 Error 1033？

生成 `trycloudflare.com` URL 不等于 tunnel 已经连通。创建 ChatGPT App 时，ChatGPT 会验证公网 MCP 端点；如果 `cloudflared` 在打印 URL 后退出，ChatGPT 可能只显示通用连接错误，Cloudflare 则可能返回 `530` / `Error 1033`。
...
把操作系统当前网络连接的 DNS 改为符合标准的解析器，例如 `1.1.1.1` 和 `1.0.0.1`，重新连接网络后再重启 CodexPro。如果 DNS 由代理客户端管理，需要开启它的系统 DNS 接管或 TUN DNS 劫持；只修改代理客户端内部的 nameserver 列表，未必会影响 `cloudflared`。
