...
| A passive read or reconnect silently starts execution | `get_goal`, `list_goals`, and `review_goal` are passive, while `refresh_goal` is store-only. Duplicate host calls are harmless and return the same authoritative state; only explicit `start_goal` or persistent resume owns execution/recovery. |
...
| Pause or cancel races with a new worker launch or integration | Pause/cancel authority is persisted before child handling. After pause linearizes no new launch, integration, or dependency advancement begins; cancel fences and terminates active workers and becomes terminal. |
...
| A failed or canceled turn advances as if it succeeded | Terminal status, result identity, content/path policy, and compact Git provenance are rechecked per turn. Failure or cancellation stops advancement and never creates an unapproved replacement attempt. |
| Goal status leaks full approved prompts or unbounded worker state | Get/review expose bounded prompt summaries, fingerprints, turn/result summaries, and compact observations; list returns compact counts/status only. Exact prompts, raw Goal records, full ledgers, and Blackboard evidence remain private local state. |
...
| An approved continuation is mistaken for a fresh retry | A continuation is a distinct mandatory turn already included in the immutable approval-time budget. Fresh automatic retries remain zero; same-operation crash recovery retains the original operation identity and cannot create another turn. |
...
- Persistent Goals must remain Isolated with an empty Goal `commands` list, network disabled, no source effects, 1–4 approved total turns per worker, and zero fresh retries. Review every ordered continuation prompt before approval; the count must equal `maxTurns - 1`, and intermediate turns cannot integrate or unlock dependencies.
- Treat same-task/thread/session continuation as identity continuity, not permission to steer a running turn or a guarantee that context compaction preserves every earlier token verbatim. A failed/canceled turn stops; do not reinterpret same-operation recovery as permission for a new attempt.
