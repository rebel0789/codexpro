...
| A passive read or reconnect silently starts execution | `get_goal`, `list_goals`, and `review_goal` are passive, while `refresh_goal` is store-only. Only explicit `start_goal` or persistent resume owns execution/recovery. |
...
| Unsupported continuation settings silently become fresh work | Persistent accepts only one turn per worker and zero fresh automatic retries. Multi-turn continuation and fresh retries fail closed; same-attempt crash recovery retains the original attempt identity. |
...
- Persistent Goals must remain Isolated with an empty Goal `commands` list, network disabled, no source effects, one worker turn, and zero fresh retries. Do not reinterpret same-attempt recovery as permission for a new attempt.
